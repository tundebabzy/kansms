...
class SmsSender():
	...
	def send(self, dest, message, buffer=False):
		...
		try:
			res = urllib2.urlopen(
				"http://%s:%d/cgi-bin/sendsms?username=%s&password=%s&charset=%s&coding=%s&to=%s&from=123&text=%s"\
...
sms = SmsSender(username='osheap', password='CHANGE-ME', port=13031)
response = sms.send('123','success')
print response
